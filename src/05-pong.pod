...
=for figure
       \includegraphics[width=0.5\textwidth]{../src/images/collision.png}
...
completely to the left of B (fig. 1.3).

=item * if A's left side is to the right of B's right side, then A is
completely to the right of B (fig 1.4).
...
We can now use it in the ball's motion handler to see if it hit any
of the paddles:
...
value between 1.5 and 2.5, or -1.5 and -2.5:
