...
=head3 Moving the Player's Paddle
...
(y) velocity. Let's add the v_y attribute in our paddle structure:
...
to do is move the paddle C<v_y> pixels at every cycle. Here is where
...
straightforward. When C<v_y> is 0 at any given cycle, the paddle won't
...
We have preset C<v_y> to zero as the paddle's initial velocity, but
...
to positive and negative velocities for our paddle
