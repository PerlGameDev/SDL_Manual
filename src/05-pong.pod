...
values of our window to 500 pixels each, set a time interval (dt) of
0.02 for the game loop, and let C<SDLx::App> handle SDL_QUIT events for us.
...
represented with L<SDLx::Rect> objects. So let's add a simple hash reference
on line 16 of the code above to store our player's paddle, right after C<$app>
instantiation.
