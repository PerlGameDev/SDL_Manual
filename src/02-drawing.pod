...
SDL surfaces coordinate system has x=0, y=0 in the upper left corner and work downward and to the right. The API always lists coordinates in x,y order.
...
Alternately color can be described as a 4 byte hexadecimal value, each two digit byte encoding the same RGBA vaues as above:
...
Pixels can also be defined as anonymous arrays as before C<[$red, $blue, $green, $alpha]>.
