...
SDL provides several ways to draw graphical elements on the screen; these
methods can be broken down into three general categories: Primitives, Images,
and Text.
Methods in each of the three categories draw a single object on the Surface.
The Surface is represented by C<SDLx::Surface>. Even our C<SDLx::App> is an
C<SDLx::Surface>. This means that we can draw directly on the C<SDLx::App>,
however there are several advantages to drawing on multiple surfaces. In this
chapter we will explore these methods of drawing, and make a pretty picture.

=head1 Coordinates

SDL's surface coordinate system has x=0,y=0 in the upper left corner and the
dimensions span to the right and downward. The API always lists coordinates in
x,y order.
More discussion of these details can be found in the SDL library documentation:
U<http://www.sdltutorials.com/sdl-coordinates-and-blitting/>
...
Drawing in SDL is done on Surfaces. The C<SDLx::Surface> object provides
access to methods in the form of:

    $surface->draw_{something}( .... );

Parameters are usually provided as array references, to define areas and
colors.

=head3 Rectangular Parameters

Some parameters are just a quick definition of the positions and dimensions.
For a rectangle that will be placed at C<(20, 20)> pixel units on the screen,
which has dimensions of C<40x40> pixel units, the following would suffice.

    my $rect = [20, 20, 40, 40];

=head3 Color

X<Color> in SDL is described by 4 numbers. The first three numbers define the
...
Alternately color can be described as a 4 byte hexadecimal value, each two
digit byte encoding the same RGBA values as above:
...
The bits of the surface are set when the C<SDLx::Surface> or C<SDLx::App> is
made.
...
All C<SDLx::Surface>s are made of pixels that can be read and written to via
a tied array interface.

    $app->[$x][$y] = $color;

The C<$color> is defined as an unsigned integer value which is constructed in
the following hexadecimal format, C<0xRRGGBBAA>.
...
Pixels can also be defined as anonymous arrays as before with
C<[$red, $green, $blue, $alpha]>.

=head2 Primitives

Drawing X<Primitives> are usually simple shapes that can be used for creating
...
Rectangles are a common building block for games. In SDL, rectangles are the most cost effective of the primitives to draw.
...
Now we will have an unfilled circle colored red and a filled circle colored blue.
...
Using our knowledge of Primitives in SDL, let's draw our field, sky, and a simple flower.
...
show them. The Surface is defined as an C<SDLx::Surface> type in SDL Perl.

=head3 Creating Surfaces

There are several ways to create an C<SDLx::Surface> for use.
...
In the event that the desired C<SDL_image> library is unavailable, we can
fall-back on the built-in support for the C<.bmp> format.
...
of a C<blit( )> function, which may not clarify as to what is going on there. Fortunately an C<SDLx::Sprite>
can be used to make our flower. Besides making drawing simpler, C<SDLx::Sprite> adds several other features
that we need for game images that move a lot. For now let's use C<SDLx::Sprite> for our flowers.
...
on the ground. Delete everything from line 34 including and after

    $flower->draw_xy( $app, 0, 0 );

and insert the code below to get a field of flowers.
